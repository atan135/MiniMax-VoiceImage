...
> 原文快照：`docs/doc_raw/api_image_t2i.html`（2026-01-28）
> 图生图（i2i）见：`docs/doc_raw/api_image_i2i.md`

使用本接口，输出文本内容，进行图片生成（即「文生图」）。当前官方文档将文生图（t2i）和图生图（i2i）拆分为两个独立页面，但二者使用 **同一个端点** `POST /v1/image_generation`，区别仅在于图生图额外支持 `subject_reference` 字段。
...
```json
{
  ...
  "id": "06254bbb1b6e81a9575bd2291e2d24ae",
  "base_resp": {
    "status_code": 0,
    "status_msg": "success"
  }
